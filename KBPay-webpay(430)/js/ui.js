$(function() {
    ...
    */
    ...
    $(".agree-terms .checkAll").not(".agree-terms .head.type02 .checkAll").keydown(function() {
        if (window.event.keyCode == 13) {
            checkAll(this);
        }
    });
    ...
    function openTipBox(obj) {
        $(obj).parents(".tool-tip").addClass("active");
        $('.tip-box').hide();
        $(obj).parents(".tool-tip").find(".tip-box").fadeIn('fast');
        if ($(obj).parents(".tool-tip").hasClass("up")) {
            $(obj).parent().next().css("bottom", 44);
        }
    }
