...
// $(function() {
    $(document).ready(function() {
        ...
        */
        ...
        // 전체동의 click
        $(".agree-terms .checkAll").not(".agree-terms .head.type02 .checkAll").click(function() {
            if ($(this).find("input").prop("checked")) {
        ...
        // 최상단 전체동의 click
        $(".agree-terms .head.type02 .checkAll").click(function() {
            if ($(this).hasClass("checked")) {
                // all check
                $(this).parent().next().find("input").prop("checked", true);
                $(this).parent().next().find(".f-chk").addClass("checked");
                $(this).parent().next().find("ul").slideUp(150);
                $(this).parent().next().find(".head .btn").removeClass("on");
            } else {
                // all check clear
                $(this).parent().next().find("input").prop("checked", false);
                $(this).parent().next().find(".f-chk").removeClass("checked");
                $(this).parent().next().find("ul").slideDown(150);
                $(this).parent().next().find(".head .btn").addClass("on");
            }
        });
        // 하위내용 체크 click
        ...
        $(".l-bottom .bottom-list button").click(function() {
            $(".form_text.curInput").addClass("on").addClass("out");
            $(".form_text.curInput input").val($(this).text()).css("opacity", 1);

            $(".curBottom").hide().removeClass("curBottom");
            $(".l-dim").hide();
            $(".curInput").find("input").focus();
        });
        ...
        if ($(".bottom-btn-area").length < 1) {
            ...
            */
            if (($(window).height() - $(".header").outerHeight()) > ($(".contents").outerHeight())) {
                $(".bottom-btn-area").not(".bottom-btn-area.static").addClass("float");
            }
        } else {    //하단버튼 o
            /*
    ...
    function agreeCombo(obj) {
        if ($(obj).hasClass("on")) {    // case : open
            //Do close
            $(obj).removeClass("on");
            $(obj).parent().next().slideUp(150);
            $(obj).text($(obj).prev().find("span").text() + '열기');

        } else {    // case : closed
            // Do open
            $(obj).addClass("on");
            $(obj).parent().next().slideDown(150);
            $(obj).text($(obj).prev().find("span").text() + '닫기');
        }
    }